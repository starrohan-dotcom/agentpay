...
async function runDemo() {
    ...
    const drawProgressBar = async (wallet: any) => {
        ...
        console.log(`ETH : [${"█".repeat(Math.min(20, ethUsed * 1000))}${"░".repeat(Math.max(0, 20 - ethUsed * 1000))}] ${ethUsed.toFixed(4)} spent`);
        console.log(`USDC: [${"█".repeat(Math.min(20, usdcUsed / 2))}${"░".repeat(Math.max(0, 20 - usdcUsed / 2))}] ${usdcUsed.toFixed(2)} spent`);
    };

    await simulateWork("Fetching market data", 0.0005, "ETH");
    await new Promise(r => setTimeout(r, 1000));

    await simulateWork("Generating AI report", 2.50, "USDC");
    await new Promise(r => setTimeout(r, 1000));

    await simulateWork("Storing results on IPFS", 0.0002, "ETH");
    await new Promise(r => setTimeout(r, 1000));

    await simulateWork("Paying API credits", 5.00, "USDC");
